...
```{r}
seda <- read.csv(file = "march_29_group_var_euclince_modelling_student_talkmove_new_achievement_survey_merged.csv")
...
seda_mth <- seda_mth %>%
  mutate(
         student_group_talkmove_ratio = mean_talk_move_counts_eucli_dist,
         student_group_utterance_ratio = mean_utterance_counts_eucli_dist,
         # tutor_talkmove_ratio = unweighted_tutor_talkmove_ratio,
         # tutor_utterance_ratio = unweighted_tutor_utterance_ratio,
         student_group_relating_ratio = mean_student_relating_counts_eucli_dist,
         student_group_asking_for_info_ratio = mean_student_asking_for_info_counts_eucli_dist,
         student_group_making_claim_ratio = mean_student_making_claim_counts_eucli_dist,
         student_group_providing_evidence_ratio = mean_student_providing_evidence_counts_eucli_dist,
         Intercept = I,
         Slope = S,
...
hist(seda_mth$student_group_talkmove_ratio)
hist(seda_mth$student_group_utterance_ratio)
hist(seda_mth$student_group_relating_ratio)
hist(seda_mth$student_group_asking_for_info_ratio)
hist(seda_mth$student_group_making_claim_ratio)
hist(seda_mth$student_group_providing_evidence_ratio)
hist(seda_mth$unique_session_IDs_count)
...
freq_table(round(seda_mth$student_group_talkmove_ratio_w, 1) )
...
Vars <- c("student_group_talkmove_ratio",  "student_group_utterance_ratio",  "student_group_relating_ratio", "student_group_asking_for_info_ratio", "student_group_making_claim_ratio", "student_group_providing_evidence_ratio")
...
```{r}
#Histogram for all the predictors
hist(seda_mth$student_group_talkmove_ratio_w)
hist(seda_mth$student_group_utterance_ratio_w)
hist(seda_mth$student_group_relating_ratio_w)
hist(seda_mth$student_group_asking_for_info_ratio_w)
hist(seda_mth$student_group_making_claim_ratio_w)
hist(seda_mth$student_group_providing_evidence_ratio_w)
hist(seda_mth$unique_session_IDs_count)
...
model_01 <- blmer(Slope ~ student_group_asking_for_info_ratio_w + unique_session_IDs_count +
